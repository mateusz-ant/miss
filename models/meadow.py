...
class Meadow(object):
    ...
    def reproduce_species(animals, new_animal_constructor):
        animal_pairs = itertools.product(filter(lambda _: random.randint(0, 9) > 3, animals), repeat=2)
    ...
    def run(self):
        while True:
            ...
            self.reproduce_species(self.env.hares, lambda: Hare(self.env, name='hare_' + seq(), display=self.display,
                                                                x=rand_x(), y=rand_y()))
            self.reproduce_species(self.env.wolves, lambda: Wolf(self.env, name='wolf_' + seq(), display=self.display,
                                                                 x=rand_x(), y=rand_y()))
